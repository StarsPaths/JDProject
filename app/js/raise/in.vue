<!--
 * @Author: huangyanming
 * @Date: 2021/01/23 10:10:51
 * @LastEditors: huangyanming
 * @LastEditTime: 2021/01/23 11:59:45
 * @Description: 描述
-->
<template>
  <Panel :class="$style.panel" title="In货推荐">
    <section :class="$style.content">
      <div :class="$style.item" v-for="item in items" :key="item.category">
          ...
          <div :class="$style.product" v-for="product in item.list" :key="product.img">
            ...
            <article>
              <div :class="$style.wrapper">
                <div :class="$style.progressBar" :style="{width:product.progress+'%'}"></div>
              </div>
              <p :class="$style.progress">{{product.progress}}%</p>
            </article>
            <label v-if="product.chose">{{product.chose}}</label>
          </div>
        </section>
      </div>
    </section>
